...
function handleJSON(json) {
  ...
  ssid = g.selectAll(".ssid").data(d3.values(ssids));
  var ssidEnter = ssid.enter().append("g").attr("class", "ssid");
  //CIRCLE
  var circle = ssidEnter.append("svg:circle")
  ...
  function filterChanged(d) {
    var changed = d.changed;
    d.changed = false;
    return changed;
  }

  var changed = ssid.filter(filterChanged);
  changed
      .attr("stroke", "#00ff00")
      .attr("stroke-width", 3)
      .transition().duration(10000)
        .attr("stroke-width", 0);
  mac.filter(filterChanged).select('.rect')
      .attr("fill", "#00ff00")
      .transition().duration(10000)
        .attr("fill", "#666666");
